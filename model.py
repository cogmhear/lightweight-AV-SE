...
class SeparatorBlock(nn.Module):
    def __init__(self, out_channels, hidden_channels, dropout=0, bidirectional=False):
        ...
        self.intra_rnn = nn.GRU(out_channels, hidden_channels, 1, batch_first=True, dropout=dropout,
                                 bidirectional=bidirectional)
        self.inter_rnn = nn.GRU(out_channels, hidden_channels, 1, batch_first=True, dropout=dropout,
                                 bidirectional=bidirectional)
    ...
    def forward(self, x, init_state=None):
        ...
        if init_state is not None:
            inter_rnn, inter_state = self.inter_rnn(inter_rnn, init_state)
        else:
...
class Separator(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels, dropout=0,
        ...
        self.separator_blocks = nn.ModuleList([SeparatorBlock(out_channels, hidden_channels, dropout=dropout,
    ...
    def forward(self, x, init_state=None):
        ...
        if init_state is not None:
            inter_states = []
            for i, l in enumerate(self.separator_blocks):
                x, state = self.separator_blocks[i](x, init_state[i:i + 1, :, :])
                inter_states.append(state)
            inter_state = torch.cat(inter_states, dim=0)
        else:
            for i, l in enumerate(self.separator_blocks):
                x = self.separator_blocks[i](x)
        x = self.prelu(x)
        x = self.conv2d(x)
        B, _, K, S = x.shape
        x = x.view(B, -1, K, S)
        ...
        x = self.activation(x)
        if init_state is not None:
            return x.transpose(0, 1)[0], inter_state
...
class AVSE(nn.Module):
    ...
    def forward(self, input):
        ...
        if state is not None:
            mask, state = self.separator(encoded_av, state)
        else:
...
class AVSEModule(LightningModule):
    def __init__(self, lr=0.00015, val_dataset=None):
